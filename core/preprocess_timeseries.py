...
def render_preprocess_timeseries(ss) -> None:
    ...
    - Lets you pick a datetime column and a numeric value column for diagnostics.
    - Quick KPIs (start/end, inferred frequency, CoV, trend slope).
    - Visuals: raw/rolling/trend lines + simple seasonality coverage.
    - Tools: parse strings → datetime, add calendar features, add rolling stats.

    Notes:
      * Robust to missing/renamed columns (falls back to safe defaults).
      * All previews are side-effect free; only "Apply" mutates the dataset.
    """
    ...
    with st.expander("Key terms (quick guide)", expanded=False):
        st.markdown(
            """
- **Inferred frequency** — Pandas’ best guess of spacing between timestamps (e.g., *D* = daily, *W* = weekly).
- **CoV (coefficient of variation)** — Std. dev. ÷ mean. >1.0 implies high volatility relative to the mean.
- **Trend slope** — Fitted linear slope (value change per row). Sign shows direction; magnitude shows strength.
- **Rolling mean** — Moving average over a fixed window (here: ~5% of series length; robust min periods).
- **Seasonality coverage** — Share of months / weekdays present (helps spot gaps or uneven sampling).
- **Calendar features** — Columns like year, quarter, month, week, weekday, etc., derived from a timestamp.
- **Rolling features** — Stats (mean/std/min/max/sum) over the last *N rows* (optionally per group).
            """
        )
    ...
    # If no dt/num columns, allow selecting any column; we will coerce safely below.
    ...
    tmp = pd.DataFrame({dt_col: s_dt, value_col: s_val}).dropna(subset=[dt_col]).sort_values(dt_col)
    ...
    if not ts.empty:
        ...
        # Simple least-squares slope on non-missing points
        y = ts.to_numpy(dtype=float)
        mask = ~np.isnan(y)
        if mask.sum() > 1:
            slope = float(np.polyfit(np.arange(len(ts))[mask], y[mask], 1)[0])
        else:
            slope = np.nan
    else:
        start = end = inferred = None
        cov = slope = np.nan
    ...
    with section("Visuals: seasonality & trend", expandable=True):
        if ts.empty:
            ...
            # Rolling window ≈ 5% of series length (with a sensible floor/ceil)
            ...
            with c1:
                ...
                if by_month.empty or by_month.max() == 0:
                    st.bar_chart(pd.Series(dtype=float))
                else:
                    st.bar_chart((by_month / by_month.max()).rename(index=str))
            with c2:
                by_wday = ts.groupby(ts.index.dayofweek).size()
                if by_wday.empty or by_wday.max() == 0:
                    st.bar_chart(pd.Series(dtype=float))
                else:
    ...
    with section("1) Parse to datetime", expandable=True):
        ...
        with c2:
            as_utc = st.checkbox("Parse as UTC", value=False, key="ts_asutc")
        with c3:
            replace_src = st.checkbox("Replace original column", value=False, key="ts_replacesrc")

        out_name_default = f"{src_col}_dt"
        out_name = st.text_input(
            "Parsed column name (if not replacing)",
            value=out_name_default,
            disabled=replace_src,
            key="ts_parse_outname",
        )
        ...
        with cprev:
            if st.button("Preview parsing", key="ts_preview_parse"):
                ...
                st.dataframe(
                    pd.DataFrame({"raw": df[src_col].head(20), "parsed": prev.head(20)}),
    ...
    with section("2) Calendar features", expandable=True):
        ...
        pick = st.multiselect(
            "Features to add",
            feats_all,
            default=["year", "quarter", "month", "dayofweek"],
            key="ts_feat_pick",
        )

        def _make_feats(idx: pd.DatetimeIndex) -> pd.DataFrame:
            """Build selected calendar features from a DatetimeIndex."""
            out = {}
            if "year" in pick:
                out["year"] = idx.year
            if "quarter" in pick:
            ...
            if "month" in pick:
                out["month"] = idx.month
            if "week" in pick:
                # Newer pandas: isocalendar() returns a DataFrame-like accessor
                try:
                    out["week"] = idx.isocalendar().week.values
                except Exception:
                    # Fallback for older versions
                    out["week"] = getattr(idx, "week", idx.isocalendar().week).astype(int)
            if "dayofweek" in pick:
                out["dayofweek"] = idx.dayofweek
            if "day" in pick:
                out["day"] = idx.day
            if "is_month_end" in pick:
                out["is_month_end"] = idx.is_month_end.astype(int)
            if "is_month_start" in pick:
    ...
    with section("3) Rolling features (row-based windows)", expandable=True):
        ...
        default_nums = num_opts[:1] if num_opts else []
        num_cols = st.multiselect("Numeric columns", num_opts, default=default_nums, key="ts_roll_nums")
        ...
        def _make_rolling(dfin: pd.DataFrame) -> pd.DataFrame:
            """Compute rolling features over row windows (optionally grouped)."""
            d = dfin.copy()
            d[order_col] = pd.to_datetime(d[order_col], errors="coerce")
            d = d.sort_values(order_col)

            if not num_cols or not windows or not funcs:
                return d
        ...
        with capply:
            if st.button("Add rolling features", key="ts_roll_apply"):
                ...
                if overwrite:
                    ...
                    # Only add new columns; keep existing untouched
